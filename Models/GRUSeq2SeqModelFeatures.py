...
class Encoder(nn.Module):
    def __init__(self, seq_len, n_features, device, embedding_dim, num_layers, dropout, bidirectional=False):
        ...
        self.rnn1 = nn.GRU(input_size=n_features, hidden_size=self.hidden_size, num_layers=self.num_layers,
...
class Decoder(nn.Module):
    def __init__(self, seq_len, input_dim, n_features, num_layers, dropout, bidirectional=False):
        ...
        self.rnn1 = nn.GRU(input_size=self.n_features, hidden_size=self.hidden_dim, num_layers=self.num_layers,
    ...
    def forward(self, x, input_hidden):
        ...
        x, hidden_n = self.rnn1(x, input_hidden)
...
class Seq2SeqGRU(nn.Module):
    ...
    def forward(self, x, prev_y, features):
        ...
        for out_days in range(self.output_length):
            prev_x, prev_hidden = self.decoder(dec_input, hidden)
            hidden = prev_hidden
            prev_x = prev_x[:, :, :24]
